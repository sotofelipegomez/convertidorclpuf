...
# --- 2. PERSISTENCIA TOTAL DEL HISTORIAL ---
# Usamos una clave global que Streamlit no toque al cambiar de pestaña
...
# Creamos un contenedor para los inputs (esto ayuda a que la página cargue desde arriba)
                        ...
                        if st.form_submit_button("Convertir"):
                            ...
                            if monto_num:
                                ...
                                st.session_state.historial_global.append({
                                    "clp": monto_num, "uf": res_uf, "ref": f"Fecha: {fecha_str} (${formato_chile(valor_uf)})"
                                })
                                st.rerun()
            ...
            with st.form("form_manual", clear_on_submit=True):
                ...
                if st.form_submit_button("Convertir"):
                    ...
                    if monto_num:
                        ...
                        st.session_state.historial_global.append({
                            "clp": monto_num, "uf": res_uf, "ref": f"UF Fija: ${formato_chile(valor_uf_fijo)}"
                        })
                        st.rerun()
...
# --- 6. SECCIÓN DE RESULTADOS (FUERA DE LAS PÁGINAS PARA PERSISTENCIA) ---
